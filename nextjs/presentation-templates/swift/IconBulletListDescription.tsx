    ...
    <>
      ...
      >
        ...
        <div className="px-12 pt-3">
          ...
          <p className="mt-3 text-[16px] max-w-[760px]" style={{ color: "var(--text-body-color, #4e4e4e)" }}>{slideData?.description}</p>
        </div>
            ...
            {features.slice(0,4).map((f, i) => (
              <div key={i} className="rounded-[22px] shadow-[0_16px_40px_rgba(0,0,0,0.08)] overflow-hidden" style={{ backgroundColor: 'var(--primary-accent-color, #BFF4FF)' }}>
                <div className="px-6 py-5">
                  ...
                  <p className="mt-3 text-[14px] leading-[1.7]" style={{ color: 'var(--text-body-color, #4e4e4e)' }}>{f.body}</p>
                </div>
              </div>
            ))}
          </div>
